...
app.patch("/api/folder", (req, res) => {
  ...
  fs.rename(
    join(__dirname, "drive", location, oldName),
    join(__dirname, "drive", location, newName),
    (e) => {
      //console.log(e)
      if (e) {
...
app.delete("/api/folder", (req, res) => {
  ...
  let fileName = decodeURI(req.body.name);
  fs.readdir(
    join(__dirname, "drive", location, fileName),
    { withFileTypes: true },
    (e) => {
      //console.log(e)
        ...
        fs.rmdir(
          ...
          (e) => {
            ...
            if (e) {
              ...
              res
                .status(200)
                .send({ message: "sucessfully removed the folder" });
            }
          }
        );
    }
  ...
  fs.readdir(
    ...
    (e, data) => {
      if (e) {
        ...
        let newData = [];
        data.forEach((e) => {
          newData.push({ name: e.name, isFile: e.isFile() });
        });
        res.send(newData);
      }
    }
...
app.post("/api/file", (req, res) => {
  ...
  form.parse(req, function (err, fields, files) {
    ...
    fs.readFile(
      join(__dirname, "drive", fields.location, fields.fileName),
      (err, data) => {
        if (err && err.errno !== -2) {
